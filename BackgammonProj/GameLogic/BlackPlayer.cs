...
namespace BackgammonLogic
{
    public class BlackPlayer : Player
    {
        ...
        internal List<int> AvailableMoves(Board gameBoard, Dice gameDice, int? playerInitialTriangleChoice)
        {
            List<int> listOptions = new List<int>();

            GetAvailableMoves(gameBoard, gameDice).Where(kv => kv.Key == playerInitialTriangleChoice)
                .ToList()
                .ForEach(kv => listOptions.Add(kv.Value));
            GetAvailableMovesEat(gameBoard, gameDice).Where(kv => kv.Key == playerInitialTriangleChoice)
               .ToList()
               .ForEach(kv => listOptions.Add(kv.Value));
            return listOptions;
        }
        ...
        internal List<int> AvailableMovesBarOff(Board gameBoard, Dice gameDice)
        {
            List<int> listOptions = new List<int>();

            GetAvailableBearOffMoves(gameBoard, gameDice)
                .ToList()
                .ForEach(kv => listOptions.Add(kv.Value));
            GetAvailableMovesEat(gameBoard, gameDice)
               .ToList()
               .ForEach(kv => listOptions.Add(kv.Value));
            return listOptions;
        }
        ...
        public override bool CanBearOffCheckers(Board gameBoard)
        {
            ...
            for (int i = 6; i <= 23; i++)
            {
                ...
                {
                    if (gameBoard.Triangles[i].NumOfCheckers > 0)
                        return false;
                }
            }
